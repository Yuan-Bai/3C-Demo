...
namespace ThreeCDemo.Locomotion.CustomKcc
{
    ...
    public sealed class PlayerKinematicMotor : MonoBehaviour
    {
        ...
        private Vector3 AdjustForGround(Vector3 displacement)
        {
            ...
            {
                // 不在可行走的斜坡上需要去除向上的分量
                Vector3 slopeDisplacement = displacement-Vector3.Dot(displacement, GroundNormal)*GroundNormal;
                displacement = Vector3.Dot(slopeDisplacement, Vector3.up) > 0 ? Vector3.zero : displacement;
                return displacement;
            }
